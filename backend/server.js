...
const dataSchema = new mongoose.Schema({
  ...
  age: Number,
  email: String,
  image: Buffer // Store images as Buffer
});
...
app.post('/api/saveData', upload.single('image'), async (req, res) => {
  try {
    ...
    const newImage = req.file ? req.file.buffer : null; // Use the buffer data
...
app.get('/api/getImage/:id', async (req, res) => {
  try {
    ...
    res.set('Content-Type', 'image/*'); // Set appropriate content type
    res.send(data.image);
  } catch (err) {
    res.status(500).send(err.message);
  }
...
// image saved in uploads file with in computers memory folder
// const express = require('express');
// const bodyParser = require('body-parser');
// const mongoose = require('mongoose');
// const multer = require('multer');
